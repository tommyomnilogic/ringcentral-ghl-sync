...
export function parseRingCentralEmail(htmlBody, subject) {
  // Strip HTML tags to get plain text, preserving list items as bullet points
  const text = htmlBody
  ...
  if (namedMatch) {
    ...
    contactName = rawName === rawName.toUpperCase()
  ...
  let summary = '';
  const introMatch = normalized.match(/at\s+\d+:\d+\s*[AP]M:?\s*([\s\S]+?)(?=__RECAP__|__TASKS__|$)/i);
  if (introMatch) {
